...
%#function predict
%#function wibble
[trackingparameters,esequence]=initializeTrackingStructures(esequence,trackingparameters,parameters);
easy = tic;
%linking
...
for offset=trackingparameters.minnondivscore:trackingparameters.nondivscorestep:trackingparameters.maxnondivscore%.875%1 %.1:.0125:.9
    ...
    esequence=greedyEndScore(esequence,trackingparameters,parameters);
   %{
    if(firstloop)
        firstloop=false;
...
removed=[];
BifurcationMeasures=[];
esequence=greedydeleteFPbranches(esequence,trackingparameters,parameters);
