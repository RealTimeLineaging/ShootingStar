function [mergedNucleiSet,e]=resolveConflicts(nucleiSet,diskSet,e,anisotropy,numcells,celldiameter,savedata,parameters)
...
if(~isempty(mergeinfo2a))
    s=size(mergeinfo2a);
    mergelower=getParameter('mergelower',numcells,[],parameters);
    mergesplit=getParameter('mergesplit',numcells,[],parameters);
    ...
    for possiblemerge=1:s(1)
        splitthreshold(possiblemerge)=getParameter('split',numcells,allpoints(mergeinfo2a(possiblemerge,1),:),parameters);
        distmergethreshold(possiblemerge)=getParameter('nndist_merge',numcells,allpoints(mergeinfo2a(possiblemerge,1),:),parameters);
        arthreshold(possiblemerge)=getParameter('armerge',numcells,allpoints(mergeinfo2a(possiblemerge,1),:),parameters);
    end
