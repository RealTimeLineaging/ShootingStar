function [diskSet,centerindicies]=createDiskSet(X,maximathreshold,zlevel,celldiameter,anisotropy,numcells,ROIpoints,ROI,ROIxmin,ROIymin,parameters,Xr,fish)
...
if fish
    Xr=imresize(Xr,.5);
    parfor i=1:size(Xr,3)
        Xr(:,:,i)=medfilt2(Xr(:,:,i),[13,13],'symmetric');
    end
    Xr=imresize(Xr,2);

    twodmax=twodmax(Xr(twodmax)>1710);%was 1740 more aggressive filtering should allow to rais it
end
...
%xymax=[y,x,z];
diskSet.xymax=[y,x,z];
[diskSet.xydetdiameters,diskSet.centeredxymax,diskSet.xycoverage,xpositions,ypositions]=calculateSphereDiameters_geometric(diskSet.xymax,X,celldiameter,true,parameters);
