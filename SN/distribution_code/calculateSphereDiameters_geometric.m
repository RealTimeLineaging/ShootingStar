...
function [diameters,centers,coverage,imagecoord_xpoints,imagecoord_ypoints]=calculateSphereDiameters_geometric(centers,Xfilt,expecteddiameter,recenter,parameters)
...
%lowenough=.25;
lowenough=getParameter('boundary_percent',length(centers),[],parameters);
...
time_to_cast_rays = toc(traycast)
...
%largerthresh=3;%1.5;
%smallerthresh=1/3;
largerthresh=getParameter('large_ray_threshold',length(centers),[],parameters);
smallerthresh=getParameter('small_ray_threshold',length(centers),[],parameters);
outlierfilter=true;
