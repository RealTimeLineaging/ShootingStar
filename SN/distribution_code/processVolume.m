...
if(isstruct(previous))
    numcells=size(previous.finalpoints,1);
else
...
sigma=celldiameter*getParameter('sigma',numcells,[],parameters);
%hack for drosophila
%maximathreshold=getParameter('intensitythreshold',time);
maximathreshold=getParameter('intensitythreshold',numcells,[],parameters);
...
nucleiSet=struct;
if(exist('Xr'))
    [diskSet,nucleiSet.centerindicies]=createDiskSet(X,maximathreshold,zlevel,celldiameter,anisotropy,numcells,ROIpoints,ROI,ROIxmin,ROIymin,parameters,Xr,fish);
else
    [diskSet,nucleiSet.centerindicies]=createDiskSet(X,maximathreshold,zlevel,celldiameter,anisotropy,numcells,ROIpoints,ROI,ROIxmin,ROIymin,parameters,[],fish);
end
time_to_castrays(init) = toc(trays);
...
%concatenated on
nucleiSet=findOverlookedNuclei(X,nucleiSet,diskSet,anisotropy,celldiameter,numcells,parameters);
clear('Xorig');
clear ('X');
%conflict resolution
[mergedNucleiSet,e]=resolveConflicts(nucleiSet,diskSet,e,anisotropy,numcells,celldiameter,savedata,parameters);

time_to_merge_nuclei(init) = toc(tnuclei);   
